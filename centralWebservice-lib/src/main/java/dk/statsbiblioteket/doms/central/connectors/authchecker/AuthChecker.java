...
public class AuthChecker {
    ...
    public User createTempUser(String username, List<String> roles)
        ...
        try {
            ...
            WebResource tmp = restApi.path("/createTempUser/")
    ...
    public User createTempAdminUser(String username, List<String> roles)
            throws BackendMethodFailedException, BackendInvalidCredsException, BackendInvalidResourceException {
        try {
            User user;

            WebResource tmp = restApi.path("/createAdminUser/")
                    .path(URLEncoder.encode(username, "UTF-8"))
                    .path("/WithTheseRoles");
            for (String role : roles) {
