...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="/" xmlns:METS="http://www.loc.gov/METS/"
                  xmlns:xlink="http://www.w3.org/1999/xlink">
        <xsl:copy>
    ...
    <xsl:template match="METS:agent[@ROLE='IPOWNER']"
                  xmlns:METS="http://www.loc.gov/METS/">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template
            match="METS:techMD|METS:rightsMD|METS:sourceMD|METS:digiprovMD|METS:descMD|METS:serviceBinding"
            xmlns:METS="http://www.loc.gov/METS/">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:apply-templates select="node()"/>
    ...
    <xsl:template match="METS:mdWrap/METS:xmlData"
                  xmlns:METS="http://www.loc.gov/METS/">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:variable name="metadataID" select="../../../@ID"/>
            <!-- e.g., DIGIPROV1, from amdSec element -->
            <xsl:choose>
                <xsl:when
                        test="$substitutions/fbm:input/fbm:metadata/fbm:metadata[@ID=$metadataID]">
                    <xsl:apply-templates
                            select="$substitutions/fbm:input/fbm:metadata/fbm:metadata[@ID=$metadataID]/node()"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when
                                test="$substitutions/fbm:input/fbm:datastreams/fbm:datastream[@ID=$metadataID]/fbm:xmlContent">
                            <xsl:apply-templates
                                    select="$substitutions/fbm:input/fbm:datastreams/fbm:datastream[@ID=$metadataID]/fbm:xmlContent/node()"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:apply-templates select="node()"/>
                        </xsl:otherwise>
    ...
    <xsl:template match="METS:mdWrap"
        ...
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:variable name="metadataID" select="../../@ID"/>
            <!-- e.g., DESC1, from METS:dmdSecFedora element -->
            <xsl:if test="$substitutions/fbm:input/fbm:metadata/fbm:metadata[@ID=$metadataID]/@LABEL">
                <xsl:attribute name="LABEL">
                    <xsl:value-of
                            select="$substitutions/fbm:input/fbm:metadata/fbm:metadata[@ID=$metadataID]/@LABEL"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="$substitutions/fbm:input/fbm:datastreams/fbm:datastream[@ID=$metadataID]/@LABEL">
                <xsl:attribute name="LABEL">
                    <xsl:value-of
                            select="$substitutions/fbm:input/fbm:datastreams/fbm:datastream[@ID=$metadataID]/@LABEL"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="$substitutions/fbm:input/fbm:metadata/fbm:metadata[@ID=$metadataID]/@MIMETYPE">
                <xsl:attribute name="MIMETYPE">
                    <xsl:value-of
                            select="$substitutions/fbm:input/fbm:metadata/fbm:metadata[@ID=$metadataID]/@MIMETYPE"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="$substitutions/fbm:input/fbm:datastreams/fbm:datastream[@ID=$metadataID]/@MIMETYPE">
                <xsl:attribute name="MIMETYPE">
                    <xsl:value-of
                            select="$substitutions/fbm:input/fbm:datastreams/fbm:datastream[@ID=$metadataID]/@MIMETYPE"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
